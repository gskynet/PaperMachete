    ...
    while menu:
        ...
        if menu_option == 1:
            ...
            if not isfile(join(ANALYSIS, binary)):
                ...
                functions = str(
                    eval(input('Specify a list of functions examine seperated by spaces (ENTER for all): '))).split()
                if len(functions) == 0:
        ...
        # migrate a json file into Grakn
            ...
            try:
                ...
                keyspaces = literal_eval(http_client.request('GET', 'http://127.0.0.1:4567/kb').data)
                ...
                print((
                    "Unable to query keyspace names. Is Grakn running?\nContinuing assuming keyspace '{}' is OK to use.".format(
                        keyspace)))
            ...
            try:
                ...
                subprocess.call(
                    ["graql", "console", "-f", join(MACHETE, "templates", "binja_mlil_ssa.gql"), "-k", keyspace])
                ...
                for num in range(1, 8):
                    ...
                    subprocess.call(["graql", "migrate", "json", "--template",
                                     join(MACHETE, "templates", "binja_mlil_ssa_{}.tpl".format(num)), "--input",
                                     join(ANALYSIS, json), "--keyspace", keyspace])

                print((
                    "Data successfully migrated into Grakn. You can now run CWE query scripts against '{}' to check for vulnerabilities".format(
                        keyspace)))
                eval(input(ENTER))
            except:
        ...
        # run CWE queries
            ...
            http_client = urllib3.PoolManager()
            keyspace = None
            keyspaces = literal_eval(http_client.request('GET', 'http://127.0.0.1:4567/kb').data)['keyspaces']
